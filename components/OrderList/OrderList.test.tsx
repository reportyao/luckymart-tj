...
describe('OrderList Component', () => {}
  ...
  describe('操作按钮测试', () => {}
    ...
    it('应该调用查看详情回调', async () => {}
      ...
      expect(mockViewDetails).toHaveBeenCalledWith((mockOrders?.0 ?? null));
    });
    ...
    it('应该调用取消订单回调', async () => {}
      ...
      expect(mockCancelOrder).toHaveBeenCalledWith((mockOrders?.1 ?? null));
    });
  });
...
describe('OrderListUtils', () => {}
  ...
  describe('filterBySearch', () => {}
    it('应该正确过滤订单', () => {}
      ...
      expect((result?.0 ?? null).order_number).toBe('ORD-2023-001');
    });
  ...
  describe('filterByStatus', () => {}
    it('应该按状态过滤订单', () => {}
      ...
      expect(result).toHaveLength(1);
      expect((result?.0 ?? null).status).toBe('pending');
    });
  ...
  describe('canCancel', () => {}
    it('应该正确判断订单是否可以取消', () => {}
      expect(OrderListUtils.canCancel((mockOrders?.1 ?? null))).toBe(true); // pending
      expect(OrderListUtils.canCancel((mockOrders?.0 ?? null))).toBe(false); // delivered
    });
  });

  describe('canRefund', () => {}
    it('应该正确判断订单是否可以退款', () => {}
      expect(OrderListUtils.canRefund((mockOrders?.1 ?? null))).toBe(false); // pending
      expect(OrderListUtils.canRefund((mockOrders?.0 ?? null))).toBe(false); // delivered
    });
  });

  describe('getTotalItems', () => {}
    it('应该计算订单商品总数', () => {}
      const total = OrderListUtils.getTotalItems((mockOrders?.1 ?? null)); // 2个商品;
      expect(total).toBe(2);
    });
  });
