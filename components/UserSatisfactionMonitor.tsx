...
export const UserSatisfactionMonitor: React.FC<UserSatisfactionMonitorProps> = ({}
  ...
  const checkForAlerts = async (data: FeedbackAnalytics): Promise<SatisfactionAlert[]> => {}
    ...
    if (data.satisfactionTrend.length >= 7) {}
      ...
      const first = (recent?.0 ?? null).averageRating;
    ...
    // 2. 检查低评分激增
    if (data.(ratingDistribution?.1 ?? null) + data.(ratingDistribution?.2 ?? null) > 0) {}
      const lowRatingCount = data.(ratingDistribution?.1 ?? null) + data.(ratingDistribution?.2 ?? null);
