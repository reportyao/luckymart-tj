...
export async function GET(request: NextRequest) {
  try {
    ...
    // 检查风控管理权限
    const hasPermission = admin.permissions.includes('risk:read') || admin.role === 'super_admin';
    if (!hasPermission) {
      return NextResponse.json({ 
        success: false,
        error: '权限不足：无法查看风控参数' 
...
export async function POST(request: NextRequest) {
  try {
    ...
    // 检查风控管理权限
    const hasPermission = admin.permissions.includes('risk:write') || admin.role === 'super_admin';
    if (!hasPermission) {
      return NextResponse.json({ 
        success: false,
        error: '权限不足：无法创建风控参数' 
    ...
    console.error('创建风控参数失败:', error);
    return NextResponse.json(
      { success: false, error: '创建风控参数失败' }, 
      { status: 500 }
    );
  }
}
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    console.error('删除风控参数失败:', error);
    return NextResponse.json(
      { success: false, error: '删除风控参数失败' }, 
      { status: 500 }
    );
  }
}