...
export async function GET(request: NextRequest) {
  try {
    ...
    // 检查功能开关查看权限
    const hasPermission = admin.permissions.includes('features:read') || admin.role === 'super_admin';
    if (!hasPermission) {
      return NextResponse.json({ 
        success: false,
        error: '权限不足：无法查看功能开关' 
...
export async function POST(request: NextRequest) {
  try {
    ...
    // 检查功能开关管理权限
    const hasPermission = admin.permissions.includes('features:write') || admin.role === 'super_admin';
    if (!hasPermission) {
      return NextResponse.json({ 
        success: false,
        error: '权限不足：无法创建功能开关' 
