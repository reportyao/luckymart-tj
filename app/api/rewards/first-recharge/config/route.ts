  ...
  try {
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        metadata: {
          supportedAmounts: FIRST_RECHARGE_REWARDS.map(((tier : any) : any) => tier.amount),
          totalRewardOptions: FIRST_RECHARGE_REWARDS.length,
          minimumRechargeAmount: Math.min(...FIRST_RECHARGE_REWARDS.map(((tier : any) : any) => tier.amount)),
          maximumRewardAmount: Math.max(...FIRST_RECHARGE_REWARDS.map(((tier : any) : any) => tier.reward))
}
      },
      meta: {
        requestId,
