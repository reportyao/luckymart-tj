...
export const GET = withAuth(async (request: NextRequest, user: any) => {
  try {
    ...
    const stats = {
      ...
      totalPossibleReward: updatedTasks.reduce((sum: any: any,   task: any: any) => sum + task.reward.amount, 0),
      availableToClaim: updatedTasks.filter(((task : any) : any) => task.canClaim).length
    };
