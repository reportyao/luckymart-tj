...
export async function GET(
  ...
  return withReadPermission(req, async (adminUser) => {
    ...
    const stats = {
      ...
      approvedPosts: posts.filter(((p : any) : any) => p.status === 'approved').length,
      rejectedPosts: posts.filter(((p : any) : any) => p.status === 'rejected').length,
      pendingPosts: posts.filter(((p : any) : any) => p.status === 'pending').length,
      totalLikes: posts.reduce((sum: any: any,   p: any: any) => sum + (p.likesCount || 0), 0),
      totalComments: posts.reduce((sum: any: any,   p: any: any) => sum + (p.commentsCount || 0), 0),
      totalViews: posts.reduce((sum: any: any,   p: any: any) => sum + (p.viewsCount || 0), 0),
      averageHotness:
        posts.length > 0
          ? Math.round(posts.reduce((sum: any: any,   p: any: any) => sum + (p.hotnessScore || 0), 0) / posts.length)
          : 0,
    };
    ...
    const prizeStats = posts;
      ...
      .reduce((acc, p) => {
        ...
        (acc?.prizeName ?? null).count++;
        (acc?.prizeName ?? null).totalValue += prizeValue;
        return acc;
      }, {} as Record<string, { count: number; totalValue: number }>);

    // 时间分布分析
    const postsByMonth = posts.reduce((acc: any: any,   p: any: any) => {
    ...
    const profile = {
      ...
      avgPostInterval:
        posts.length > 1
          ? Math.round(
              (new Date((posts?.0 ?? null).createdAt).getTime() -
