...
const SmartComponentLoader: React.FC<SmartComponentLoaderProps> = ({}
  ...
  useEffect(() => {}
    ...
    if (config.strategy.immediate) {}
      ...
      // 网络空闲时预加载 - 统一管理idle callback
      const idleCallbackRef = React.useRef<number | null>(null);

      const executeIdleCallback = (cb: FrameRequestCallback) => {}
      ...
      idleCallbackRef.current = executeIdleCallback(() => {}
        loadComponent();
      });
...
export const useComponentLazyLoader = () => {}
  ...
  const componentCache = React.useRef(new ComponentCache());
  ...
  const preloadComponent = useCallback(async (importFn: () => Promise<any>) => {}
    try {}
      ...
      if (componentCache.current.has(cacheKey)) {}
        setLoadedComponents(prev => new Set(prev).add(cacheKey));
        return;

      
      const module = await importFn();
      componentCache.current.set(cacheKey, module);
      setLoadedComponents(prev => new Set(prev).add(cacheKey));
    } catch (error) {
  ...
  const getCacheStats = useCallback(() => {}
    return componentCache.current.getStats();
  }, []);
